...
<h2><img src="imagenes/permisosmenu.png"  alt="permisosmenu"> Administraci&oacute;n de Estados de las Ausencuas</h2>
...
echo('
<form method="post"/>
 <div id="error"> ADVERTENCIA: Una vez pulsado el bot&oacute;n Modificar, todas las Ausencias con el estado seleccionado, 
...
include("conectarbbdd.php");
 ...
 $result = mysql_query("SELECT * FROM  `t_estadosaus`");
...
elseif (empty($nombreestado)) {
	...
	$result = mysql_query("UPDATE `t_estadosaus` SET `descestado` = '$nombreestado'  WHERE `t_estadosaus`.`idestado` = '$idestado'; ");
	if($result)
	{
...
<tr> <th colspan="4"> <b> Crear Nuevo Estado de Ausencia: </b> </th> </tr>
<tr><td>&nbsp </td></tr>
<form method="post"/>

 <tr><td colspan="4"><b>Datos del Estado del Ausencia: </b></td></tr>
<tr><td>&nbsp</td></tr>
 <tr> 
  		  <td> <label> Nombre del Estado del Ausencia: </label></td>
 </tr>
<tr>
   	 <td colspan="4"> <textarea cols="72" rows="1" name="descestado"> </textarea></td> 
</tr>
<tr>
	<td colspan="4"> <center><INPUT type="submit" name="altaestado" value="Crear Nuevo Estado de Ausencia" size="20"> </center></td>
</tr>
 </table>
 </form>
...
if (empty($descestado)) {
	...
	include("conectarbbdd.php");
	$result = mysql_query("INSERT INTO `t_estadosaus` (`idestado`, `descestado`) VALUES (NULL, '$descestado'); ");
	if($result)
	{
