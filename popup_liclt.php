...
<script> 
function retornavalores(idemp, nomb, ape1, ape2, dnie){
   window.opener.document.getElementById('idcliente').value=idemp; 
...
while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
    ...
    <img src='./imagenes/seleccionar2.png' > </button> </td> </tr>",$row[0], urlencode($row[1]), urlencode($row[2]),urlencode($row[3]), $row[5]);
}
